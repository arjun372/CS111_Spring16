...
TEST8_RESULT=1
if grep -q "sleep" test8outmessage.txt
then
    true
else
    TEST8_RESULT=0
fi
if grep -q "only" test8outmessage.txt
then
    TEST8_RESULT=0
fi
if [ $TEST8_RESULT == 1 ]
...
(time ./simpsh --rdonly test9in.txt --wronly test9out.txt --wronly test9err.txt \
    --command 0 1 2 sleep 61) 2> test9time.txt > /dev/null
TEST9_TIME_EXIST=1
if grep -q "real" test9time.txt
then
...
then
    if grep -q "1m" test9time_real.txt
    then
        NUM_FAILED=`expr $NUM_FAILED + 1`
